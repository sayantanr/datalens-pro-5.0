...
def generate_more_stats_dashboard(df, dimensions, measures, dates):
    ...
    with ai_tab:
        ...
        # Check if pandasai is installed
        ...
        if pandasai_installed:
            ...
            api_key = st.text_input("Enter OpenAI API Key (optional - leave blank to use Offline Smart Parser)", type="password", key="pandasai_api_key")
            query = st.text_input("Ask a question about your data:", placeholder="e.g. 'Show me the average sales grouped by region'", key="pandasai_query")
            ...
            if query:
                if api_key:
                    ...
                    # Run our offline smart parser fallback
                    run_offline_parser(df, query, dimensions, measures)
        else:
            st.info("💡 Running in **Offline Smart Parser Mode** (zero setup required).")
            query = st.text_input("Ask a question about your data:", placeholder="e.g. 'average sales by region' or 'show rows where Profit > 500'", key="offline_query")
            if query:
                run_offline_parser(df, query, dimensions, measures)

    with automl_tab:
        st.markdown("### 🚀 AutoML Leaderboard")
        st.write("Select a target variable and features, and DataLens will automatically train, tune, and evaluate multiple algorithms to find the absolute best model.")
        
        if not measures:
            st.info("No measures available for machine learning.")
            return
            
        target = st.selectbox("Target Variable (Measure to predict)", measures, key="automl_target")
        features = st.multiselect("Predictor Features", [m for m in measures if m != target] + dimensions, default=[m for m in measures[:4] if m != target], key="automl_features")
        ...
        if run_automl:
            ...
            with st.spinner("Running AutoML pipeline (training multiple algorithms)..."):
                # Prepare data
                df_clean = df[[target] + features].dropna()
                if len(df_clean) < 30:
                    st.warning("Not enough clean rows (need at least 30 non-null rows).")
                    return
                
                # Sample for speed if huge
                df_clean = df_clean.sample(min(10000, len(df_clean)), random_state=42)
    ...
    "Gaussian Process Regressor": GaussianProcessRegressor()
                ...
                ld_df = pd.DataFrame(leaderboard).sort_values(by="R² Score", ascending=False).reset_index(drop=True)
                
                # Highlight best model
                best_model_name = ld_df.iloc[0]["Algorithm"]
                best_r2 = ld_df.iloc[0]["R² Score"]
                
                st.success(f"🏆 **Winner: {best_model_name}** with R² score of **{best_r2:.3f}**!")
                
                # Show Leaderboard Table
                st.dataframe(ld_df.style.background_gradient(subset=["R² Score"], cmap="Greens").background_gradient(subset=["Mean Absolute Error (MAE)", "Root Mean Squared Error (RMSE)"], cmap="Reds"), use_container_width=True)
                
                # Visualize leaderboard
                fig = px.bar(ld_df, x="Algorithm", y="R² Score", color="R² Score", color_continuous_scale="Viridis", title="Model Performance Leaderboard (R² Score)")
                st.plotly_chart(fig, use_container_width=True, key="automl_leaderboard_chart")
